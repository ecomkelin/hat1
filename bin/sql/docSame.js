	...
	try {
		...
		for(key in doc) {		// 循环文档中的每个field
			...
			if(doc[key].unique) {								// 判断field是否为unique 如果是unique 则不需要判断其他的
				...
				if(!(uniq instanceof Array)) return reject({errMsg: `${type} 数据库 doc 的uniq值错误`});
				param[key] = docNew[key];			// 相当于 {code: '员工编号'}
				for(let i=0; i<uniq.length; i++) {
					let sKey = uniq[i];
					if(docNew[sKey] === undefined) return reject({errMsg: `${type} 请传递 在新的 doc中传递 [${key}] 的值`});
				...
				if(!(true_uniq instanceof Array)) return reject({errMsg: `${type} 数据库 doc 的true_uniq值错误`});
